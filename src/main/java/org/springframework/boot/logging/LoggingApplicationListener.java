...
public class LoggingApplicationListener implements GenericApplicationListener {
	 ...
	 * written.
	 */
	public static final String PATH_PROPERTY = org.springframework.boot.logging.LogFile.PATH_PROPERTY;

	/**
	 * The name of the Spring property that contains the name of the log file. Names can
	 * be an exact location or relative to the current directory.
	 */
	public static final String FILE_PROPERTY = org.springframework.boot.logging.LogFile.FILE_PROPERTY;
...
//		environment.getSystemProperties().put("LOG_PATH", "d:\\\\logs\\\\ibd-notify2");
//		environment.getSystemProperties().put("LOG_FILE", "ibd-notify2");

		new LoggingSystemProperties(environment).apply();
		org.springframework.boot.logging.LogFile logFile = org.springframework.boot.logging.LogFile.get(environment);
		if (logFile != null) {
	...
	private void setLogLevel(LoggingSystem system, Environment environment, String name,
		...
		try {
			...
			// 如果自定义的业务日志级别存在，则用自定义的日志级别。 modify source by roy
			if ("com.shengjing".equals(name)) {
				if (StringUtils.hasLength(CustomProperties.getInstance().getLoggerLevel())) {
					level = CustomProperties.getInstance().getLoggerLevel();
				}
			}
