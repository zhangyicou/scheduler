...
public class LogFile {
	...
	/**
	 * Create a new {@link org.springframework.boot.logging.LogFile} instance.
	...
	/**
	 * Get a {@link org.springframework.boot.logging.LogFile} from the given Spring {@link Environment}.
	 * @param propertyResolver the {@link PropertyResolver} used to obtain the logging
	 * properties
	 * @return a {@link org.springframework.boot.logging.LogFile} or {@code null} if the environment didn't contain any
	 * suitable properties
	 */
	public static org.springframework.boot.logging.LogFile get(PropertyResolver propertyResolver) {
		String file = propertyResolver.getProperty(FILE_PROPERTY);
		String path = propertyResolver.getProperty(PATH_PROPERTY);

		// 如果自定义的日志路径，存在则用自定义的日志路径。 modify source by roy
		if (StringUtils.hasLength(CustomProperties.getInstance().getLoggerPath())) {
			path = CustomProperties.getInstance().getLoggerPath();
		}

		if (StringUtils.hasLength(file) || StringUtils.hasLength(path)) {
			return new org.springframework.boot.logging.LogFile(file, path);
		}
		return null;
	}
